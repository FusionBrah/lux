defmodule Lux.Prisms.Binance.Futures.BinanceFuturesCancelOrderPrism do
  @moduledoc """
  A prism that cancels futures orders on the Binance exchange.

  ## Example

      # Cancel by order ID
      iex> Lux.Prisms.Binance.Futures.BinanceFuturesCancelOrderPrism.run(%{
      ...>   symbol: "BTCUSDT",
      ...>   order_id: 123456
      ...> })
      {:ok,
  ...
  use Lux.Prism,
    ...
    description: "Cancels futures orders on Binance exchange",
    input_schema: %{
      type: :object,
      properties: %{
        symbol: %{
  ...
  defp cancel_order(api_key, api_secret, base_url, params) do
    python_result =
      python variables: %{
        ...
        ~PY"""
        from binance_utils.futures import get_futures_client, cancel_futures_order
        ...
        result = cancel_futures_order(client, symbol, order_id, orig_client_order_id)
